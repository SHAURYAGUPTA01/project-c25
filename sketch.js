...
function showArrows(index, arrows) {
  ...
  if (arrows.body.velocity.x > 0 && arrows.body.position.x > 400) {
    var position = [arrows.body.position.x, arrows.body.position.y];
    trajectory.push(position);
  }

  for (var i = 0; i < this.trajectory.length; i++) {
    fill("white");
    ellipse(trajectory[i][0], this.trajectory[i][1], 5, 5);
  }
